...
const ModalViewDocument3 = ({ closeModal, idUser, token }) => {
    ...
    useEffect(() => {
        const fetchDocument = async () => {
            setLoading(true);
            try {
                const response = await axios.get('http://localhost:8115/tetanus_vaccine_certificate/get_document', {
    ...
    const downloadDocument = async () => {
        try {
            ...
            const response = await axios.get('http://localhost:8115/tetanus_vaccine_certificate/download_image', {
                params: { pIdUser: idUser },
                headers: {
                    Authorization: `Bearer ${token}`,
                },
...
export default ModalViewDocument3;