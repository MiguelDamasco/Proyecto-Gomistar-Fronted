...
const ModalViewDocumentShip2 = ({ closeModal, idShip, token }) => {
    ...
    useEffect(() => {
        const fetchDocument = async () => {
            setLoading(true);
            try { 
                const response = await axios.get(`${myAPI}/certificate_navigability/get_document`, {
    ...
    const downloadDocument = async () => {
        try {
            ...
            const response = await axios.get(`${myAPI}/certificate_navigability/download_image`, {
                params: { pIdShip: idShip },
                headers: {
                    Authorization: `Bearer ${token}`,
                },
